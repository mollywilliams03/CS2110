...
START_TEST(test_realloc_initial_metadata) {
    ...
    unsigned long *trailing_canary = (unsigned long *)((uint8_t *)meta + meta->size - sizeof (unsigned long) + TOTAL_METADATA_SIZE);
...
START_TEST(test_realloc_initial_lists) {
    ...
    ck_assert_int_eq(size_list -> size, SBRK_SIZE - 1024 - (TOTAL_METADATA_SIZE * 2));
...
void setup_realloc_copy(void) {
    ...
    meta->size = 55;
    unsigned long *tail_canary = (unsigned long *)((uint8_t *)meta + meta->size - sizeof (unsigned long) + TOTAL_METADATA_SIZE);
    meta->canary = *tail_canary = ((uintptr_t)meta ^ CANARY_MAGIC_NUMBER) + 1890;
...
START_TEST(test_realloc_copy_smaller_retvalue) {
    ...
    unsigned long canary_expected = ((uintptr_t)ret_meta ^ CANARY_MAGIC_NUMBER) + 1890;
    ck_assert_int_eq(ret_meta->canary, canary_expected);
    unsigned long *trailing_canary = (unsigned long *)((uint8_t *)ret_meta + ret_meta->size - sizeof (unsigned long) + TOTAL_METADATA_SIZE);
    ck_assert_int_eq(*trailing_canary, canary_expected);
...
START_TEST(test_realloc_free_insert) {
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
    ck_assert_int_eq(B -> size,  b_size + 160 + c_size + 2*TOTAL_METADATA_SIZE);
    ck_ptr(B, prev_b);
}
END_TEST
...
void setup_realloc_bad_end_canary(void) {
    ...
    meta -> canary = ((uintptr_t)meta ^ CANARY_MAGIC_NUMBER) + 1890;
    unsigned long *trailing_canary = (unsigned long *)((uint8_t *)meta + meta->size - sizeof (unsigned long) + TOTAL_METADATA_SIZE);
...
void setup_realloc_bad_meta_canary(void) {
    ...
    meta -> canary = 0xBEEF;
    unsigned long *trailing_canary = (unsigned long *)((uint8_t *)meta + meta->size - sizeof (unsigned long) + TOTAL_METADATA_SIZE);
    *trailing_canary = ((uintptr_t)meta ^ CANARY_MAGIC_NUMBER) + 1890;
