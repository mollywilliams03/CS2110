...
void create_situation_1(metadata_t **Aout, metadata_t **Bout, metadata_t **Cout) {
    // Setup C
    metadata_t *C = (metadata_t *)(my_sbrk_fake_heap + 1216);
    C->size = SBRK_SIZE - 1216 - TOTAL_METADATA_SIZE;
...
START_TEST(test_malloc1_retvalue) {
    ...
    // Check return value
    ck_assert(ret);
    uint8_t *split_right_choice =  my_sbrk_fake_heap + SBRK_SIZE - 128 - sizeof(unsigned long) ;
//	ret = SUBPTR(ret);
...
START_TEST(test_malloc1_metadata) {
    ...
    unsigned long canary_expected = ((uintptr_t)meta ^ CANARY_MAGIC_NUMBER) + 1890;
    ck_assert_int_eq(meta->canary, canary_expected);
    unsigned long *trailing_canary = (unsigned long *)((uint8_t *)meta + meta->size + sizeof(metadata_t));
...
//checking to see if size_list is perfectly perfect
START_TEST(test_malloc1_lists) { 
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
    ck_assert(size_list);
    ck_assert(!size_list -> next);
    ck_assert_int_eq(size_list->size, SBRK_SIZE - 128 - (TOTAL_METADATA_SIZE * 2));
...
void malloc_initial_sbrk_setup(void) {
    ...
    size_list->size = SBRK_SIZE - TOTAL_METADATA_SIZE;
    size_list->next = NULL;
...
//    call_freelist("actual/Malloc_Init_Sbrk.txt");
    ck_assert(ret);
    uint8_t *split_right_choice =  my_sbrk_fake_heap + 1976;
    ck_ptr(ret,  split_right_choice);
}
END_TEST
...
//checks to see if size_list are correct
START_TEST(test_malloc_initial_sbrk_lists) {
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
    ck_assert(size_list);
    ck_assert(!size_list->next);
    ck_assert_int_eq(size_list->size, SBRK_SIZE - 64 - 2 * TOTAL_METADATA_SIZE);
    ck_ptr(size_list,  my_sbrk_fake_heap);
...
START_TEST(test_malloc_perf_block1_metadata) {
    ...
    ck_assert(!meta -> next);
    unsigned long expected_canary = ((uintptr_t)meta ^ CANARY_MAGIC_NUMBER) + 1890;
    ck_assert_int_eq(meta -> canary, expected_canary);
    unsigned long* ending_canary = (unsigned long*)((uint8_t*)meta + meta->size + sizeof(metadata_t));
    ck_assert_int_eq(*ending_canary, expected_canary);
}
END_TEST

//checks the size_list
...
//checks to see if size_list is correct
...
START_TEST(test_malloc_split_block1_retvalue) {
    ...
    uint8_t *split_right = my_sbrk_fake_heap + 760;
...
START_TEST(test_malloc_split_block2_retvalue) {
    ...
    ck_assert(ret);
    //metadata_t* meta = (metadata)ret;
    uint8_t *split_right = my_sbrk_fake_heap + 1816;
    ck_ptr(ret,  split_right);
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
}
END_TEST
...
START_TEST(test_malloc_split_block2_splitblock) {
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
    //make sure the address is correct
    uint8_t *split_left = (uint8_t*)my_sbrk_fake_heap + 1216;
    ck_ptr(C,  split_left);
    //check to see if the remainder is correct in the list
    unsigned long c_size = 544;
...
START_TEST(test_malloc_split_block2_metadata) {
    ...
    ck_assert(!meta -> next);
    //checks to make sure size is correct
    ck_assert_int_eq(meta -> size, 224);
    //checks to make sure canaries are correct
    unsigned long canary_expected = ((uintptr_t)meta ^ CANARY_MAGIC_NUMBER) + 1890;
    ck_assert_int_eq(meta -> canary, canary_expected);
    unsigned long *trailing_canary = (unsigned long *)((uint8_t *)meta + meta -> size + sizeof(metadata_t));
...
START_TEST(test_malloc_split_block3_retvalue) {
    ...
    ck_assert(ret);
    //metadata_t* meta = (metadata)ret;
    uint8_t *split_right = my_sbrk_fake_heap + 1433;
    ck_ptr(ret,  split_right);
    ck_assert_int_eq(my_malloc_errno, NO_ERROR);
}
END_TEST
